...
namespace dd_biot
{
    ...
    struct DataStruct
    {
        std::string name; //file name of porosity file
        std::string name2;// file name of permeabiity file
        std::vector<unsigned int> reps; //number of cells in each direction of domain
        std::vector<Point<dim>> pts; //diagonally opposite points of main domain
    };
    ...
    {
    public:
        LameParameters (const DataStruct<dim> &ds)
                : Function<dim>(),
                  ym(ds.name, ds.reps, ds.pts),
                  nu(0.2)
        {
            ym.output_field();
            ym.output_porosity_field();
        }
...
//        Permeability<dim> prmblty;
                                            ...
                                            Vector<double>   &values) const
    {
        ...
        double E = ym.get_value(p);

        switch (dim)
        {
            case 2:
//                if (y > 3.0/16.0 && y < 5.0/16.0)
//                {
//                    values(0) = 100.0;
//                    values(1) = 100.0;
//                } else if (y > 11.0/16.0 && y < 13.0/16.0)
//                {
//                    values(0) = 100.0;
//                    values(1) = 100.0;
//                } else
...
//    }
    ...
    {
    public:
      KInverse (const DataStruct<dim> &ds)
    : TensorFunction<2,dim>(),
	  prmblty(ds.name2, ds.reps, ds.pts)
	  {
    	  prmblty.output_field();
	  }
...
//        const double y = points[p][1];
        double k_value = prmblty.get_value(points[p]);
...
//            z = p[2];
//
//        const LameParameters<dim> lame_function(const DataStruct<dim> &ds);
//        Vector<double> vec(2);
...
//          return exp(t)*(cos(y*3.141592653589793)*sin(x*3.141592653589793)+1.0E1);
    ...
    {
    public:
        ExactSolution() : Function<dim>(dim*dim + dim + 0.5*dim*(dim-1) + dim +1) {}
                                         ...
                                         std::vector<Tensor<1,dim,double> > &grads) const
    {
        ...
        int total_dim = dim*dim + dim + static_cast<int>(0.5*dim*(dim-1)) + dim +1;
        Tensor<1,dim> tmp;
        switch (dim)
        {
        case 2:
...
//
////      const
//		  LameParameters<dim> lame_function(const DataStruct<dim> &ds);
//      Vector<double> vec(2);
////      lame_function.vector_value(p,vec);
//
//
